...
window.addEventListener("load", async function () {
    let result = await fetch("/api/gateway");
    API = await result.text();
...
/**
 * Seite zur Auswahl eines Golfplatzes aufrufen
 */
async function showGolfcoursePage() {
    // Golfplätze abrufen
    GOLFCOURSE = {};

    let golfcourses = fetch(`${API}/golfcourse/course`).then(result => golfcourses = result);
    let countGames = fetch(`${API}/scorecard/golfcourse/countByCourseId`).then(result => countGames = result);
    await Promise.all([golfcourses, countGames]);

    golfcourses = golfcourses.json().then(json => golfcourses = json);
    countGames = countGames.json().then(json => countGames = json);
    await Promise.all([golfcourses, countGames]);
        
    // Golfplätze anzeigen
    let pageElement = document.querySelector("#page-golfcourse");
    let golfcoursesElement = pageElement.querySelector(".golfcourses");
    golfcoursesElement.innerHTML = "";

    golfcourses._embedded.courses.forEach(course => {
        let aElement = document.createElement("div");
        aElement.classList.add("card");
        aElement.dataset.courseId = course.id;
        aElement.href = "#";
        aElement.setAttribute("aria-current", "true");
        aElement.setAttribute("class", "list-group-item list-group-item-action d-flex w-100 justify-content-between align-items-start");
        golfcoursesElement.appendChild(aElement);

        let count = countGames.find(e => e.courseId == course.id)?.count || 0;
        ...
        if (count) {
            aElement.innerHTML += `<div class="badge bg-primary rounded-pill">${count}</div>`;
        }

        aElement.addEventListener("click", async function () {
            GOLFCOURSE = course;

            document.querySelectorAll(".golfcourse-name").forEach(e => e.textContent = course.name);
            document.querySelectorAll(".golfcourse-address").forEach(e => e.textContent = course.address);

            showOverviewPage();
        });
    });

    switchPage("golfcourse");
}
...
/**
 * Übersicht der vorhandenen Scorelarten aufrufen.
 */
async function showOverviewPage() {
    // Scorekarten abrufen
    let result = await fetch(`${API}/scorecard/scorecard/search/findByGolfCourseId?golfCourseId=${GOLFCOURSE.id}`);
    ...
    json._embedded.scorecards.forEach(scorecard => {
        ...
        fetch(`${API}/scorecard/scoring/${scorecard.id}`).then(result => {
            return result.json();
        }).then(scoring => {
    ...
    // Event Handler zur Auswahl eines anderen Golfplatzes registrieren
    pageElement.querySelector("button.switch-golfcourse").addEventListener("click", async function () {
        showGolfcoursePage();
    });
}
...
async function deleteScorecard(id) {
    ...
    fetch(`${API}/scorecard/scorecard/${id}`, {method: "DELETE"});
...
async function showScorecard(id) {
    ...
    if (id > 0) {
        scorecard = fetch(`${API}/scorecard/scorecard/${id}`).then(result => scorecard = result);
        holes = fetch(`${API}/scorecard/scorecard/${id}/holes`).then(result => holes = result);
    ...
    scorecardForm.elements.course.value = scorecard.golfCourse || GOLFCOURSE.name;
    ...
    pageElement.querySelector("button.save").addEventListener("click", saveScorecard);
    pageElement.querySelector("button.save-and-quit").addEventListener("click", saveAndLeaveScorecard);
    pageElement.querySelector("button.cancel").addEventListener("click", dismissScorecard);
    ...
    if (removeButton) {
        removeButton.addEventListener("click", async () => {
            ...
            if (trElement.dataset.status !== "new") {
                fetch(`${API}/scorecard/hole/${trElement.dataset.hole}`, {method: "DELETE"});
            }
...
async function saveScorecard() {
    ...
    if (scorecardForm.dataset.status === "new") {
        if (!scorecardForm.elements.date.value || !scorecardForm.elements.time.value) {
            popupToConfirm({
        ...
        let result = await fetch(`${API}/scorecard/scorecard`, {
            method: "POST",
            headers: {
    ...
    for (let i = 0; i < holesTrCollection.length; i++) {
        ...
        hole = {
            ...
            scorecard: `${API}/scorecard/scorecards/${scorecardId}`,
        ...
        if (trElement.dataset.hole < 1) {
            let result = await fetch(`${API}/scorecard/hole`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
            ...
            let result = await fetch(`${API}/scorecard/hole/${trElement.dataset.hole}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json"
                },
 ...
 */
async function saveAndLeaveScorecard() {
    if (await saveScorecard()) {
        showOverviewPage();
    }
}
...
/**
 * Scorekarte ohne die Änderungen sichern verlassen.
 */
async function dismissScorecard() {
    let decision = await popupToConfirm({
        title: "Socrekarte verlassen",
        message: "Wollen Sie wirklich alle Änderungen verwerfen und die Scorekarte verlassen?",
        confirm: "Ja",
        dismiss: "Nein",
    });
